	...
	public void getPhoneticPattern(String pattern) {
		...
		try {
						...
						if (previousConsonantFlag == 1
							...
							if (Integer.valueOf(currentconsonantflag) == 0) {
								...
								if (previousChar.compareTo("t") == 0
										&& pattern.compareTo("h") == 0 && previousConsonantFlag == 1) {
									putbkspace();
									putbkspace();
						...
						if (ucodeValue.length() > 4) {
							for (int j = 0; j < ucodeValue.length(); j++) {
								...
								if (temp.length() == 4) {
									...
									ob.OutputActiveWindow("U"+temp.toString());
									temp.delete(0, 4);
								}
							}
							...
							if (aflag == 0) {
								ob.OutputActiveWindow("U"+ucodeValue);
							} else
								aflag = 0;
						}
			...
			System.out.println("Select a keyboard layout first!!");
             	  ...
             	  */
            	  ob.OutputActiveWindow("U"+ Integer.toHexString(unDef));
        	}
		  catch (Throwable t) {
			t.printStackTrace();
		}
