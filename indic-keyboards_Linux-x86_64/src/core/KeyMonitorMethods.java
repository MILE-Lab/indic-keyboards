...
public class KeyMonitorMethods {
	...
	static {
		...
		try {
			System.load(path + "/libKeyMonitor-64.so.1.0");
			/*
			 * Load the library if AutoRepeat is set to 0 (OFF mode)
			 * in preferences.conf
			 */
			if (IndicKeyboards.AutoRepeat == false) {
				System.load(path + "/libKeyRepeat-64.so.1.0");
			}

		} catch (UnsatisfiedLinkError e) {
			System.load("/usr/lib/libKeyMonitor-64.so.1.0");
			/*
			 * Load the library if AutoRepeat is set to 0 (OFF mode)
			 * in preferences.conf
			 */
			if (IndicKeyboards.AutoRepeat == false) {
				System.load("/usr/lib/libKeyRepeat-64.so.1.0");
			}
